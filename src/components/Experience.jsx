        ...
        <div className="relative">
          ...
          <div className="space-y-12 md:space-y-24">
            {experienceData.experiences.map((exp, idx) => (
              <div
                key={idx}
                className={`relative grid md:grid-cols-2 gap-6 md:gap-16 items-start group`}
              >
                ...
                >
                  ...
                  <p className="text-base md:text-lg font-medium text-slate-600 dark:text-slate-400 mt-1 mb-4">
                    {exp.company}
                  </p>
                  <div
                    className={`flex flex-wrap ${idx % 2 === 0 ? "md:justify-end" : "justify-start"} gap-2`}
                  >
                    {exp.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-3 py-1 text-[0.65rem] font-mono font-bold uppercase tracking-wider rounded-md border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400"
                      >
                        {tag}
                      </span>
                    ))}
